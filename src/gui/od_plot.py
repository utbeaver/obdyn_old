...
#
# Created:     2003/11/03
# RCS-ID:      $Id: od_plot.py,v 1.4 2006/10/02 01:20:57 psong Exp $
# Copyright:   (c) 2002
...
class PPTPlot(wx.Frame):
    def __init__(self, parent, _model, id, title):
        ...
        self.tree = Od_TreeCtrlPanel(self.split, self.model, self)
        self.tree.SetModel(self.model)
        self.tree.OnUpdate()
        self.split.SetMinimumPaneSize(5)
        self.client = PlotCanvas(self.split)
        self.split.SplitVertically(self.tree, self.client)
    ...
    def drawObj(self, mobj, type):
        types = mobj.get_plotable()
        if not type in types: return
        _datas = mobj.datas[type]
        _time = self.model.time
        timeData = zip(_time, _datas)
        line = PolyLine(timeData, legend=type, colour="blue")
        obj = PlotGraphics([line], type, "Time", type)
        self.client.Draw(obj)
...
##            frame = PPTPlot(None, -1, "PlotCanvas")
##            #frame.Show(True)
